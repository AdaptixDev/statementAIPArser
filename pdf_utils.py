...
class PDFConverter:
    @staticmethod
    def pdf_to_images(pdf_path: str, output_dir: str, dpi: int = 300) -> Tuple[ImageData, List[ImageData]]:
        """
        ...
        # Create output directory only if we are writing to disk.
        if Config.ENABLE_FILE_STORAGE and not os.path.exists(output_dir):
            logging.info(f"Creating output directory: {output_dir}")
            os.makedirs(output_dir)

        # Explicit poppler path for Windows - update with the correct path as needed.
        ...
        try:
            # Increase DPI for high resolution images.
            ...
            # Two branches based on whether we want to persist files to disk.
            if Config.ENABLE_FILE_STORAGE:
                ...
                for i, image in enumerate(images):
                    if i == 0:
                        pseudo_name = f"{os.path.splitext(os.path.basename(pdf_path))[0]}_front.jpg"
                    else:
                        pseudo_name = f"{os.path.splitext(os.path.basename(pdf_path))[0]}_page_{i+1}.jpg"
    
                    bio = BytesIO()
                    image.save(bio, format='JPEG', quality=100, optimize=False)
                    image_bytes = bio.getvalue()
                    logging.info(f"Converted image {i+1} stored in memory with indicator: {pseudo_name}")
