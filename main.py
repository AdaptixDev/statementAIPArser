...
def process_single_file(file_path: str, client: AssistantClient) -> None:
    ...
    Process a single PDF file by:
      ...
      2. Sending each image (except the front page) to OpenAI for processing in parallel batches.
      ...
      5. Merging the final results into one JSON file (saved to disk).
    """
    ...
    try:
        ...
        # Remove the front page from the batch processing to avoid duplicate processing.
        if first_page:
            print("[INFO] Front page image detected; processing it separately.")
            images = images[1:]
